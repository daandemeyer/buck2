    ...
    pub fn add_copied(
        ...
        let entry = match src_value.entry() {
            ...
            DirectoryEntry::Leaf(ActionDirectoryMember::Symlink(s)) => {
                ...
                if relative_symlinks && (s.target().is_empty() || orig_dest.starts_with(src)) {
                    let s = if s.target().is_empty() {
                        Arc::new(Symlink::new(".".into()))
                    } else {
                        s.dupe()
                    };
                    DirectoryEntry::Leaf(ActionDirectoryMember::Symlink(s))
...
mod tests {
    ...
    #[test]
    fn copy_lone_empty_symlink_target_as_current_directory() -> buck2_error::Result<()> {
        for (relative_symlinks, expected) in [(false, "../../source"), (true, ".")] {
            let fs = ProjectRootTemp::new().unwrap();
            let mut builder = ArtifactValueBuilder::new(fs.path(), DigestConfig::testing_default());
            let entry = builder.add_copied(
                &get_symlink_artifact_value(""),
                path("source/self"),
                path("buck-out/copied/self"),
                None,
                relative_symlinks,
            )?;

            let symlink = match entry.as_ref() {
                DirectoryEntry::Leaf(ActionDirectoryMember::Symlink(symlink)) => symlink,
                _ => panic!("Symlink type is expected!"),
            };
            assert_eq!(symlink.target().as_str(), expected);
        }
