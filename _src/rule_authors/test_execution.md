...
timed-out executions are not cached, and neither are executions that declare
outputs, since a cached result does not recreate them.
...
`--no-remote-cache` does not bypass DICE caching.
